"""A stand-in for the native engine, so the HTTP surface is testable offline.

Kept out of ``conftest.py`` on purpose: this module imports ``needle_openai``,
and the live suite runs against a container with only ``pytest`` and ``httpx``
installed. ``conftest`` is loaded for every test session, so anything it imports
at module scope becomes a hard dependency of the live suite too.
"""
